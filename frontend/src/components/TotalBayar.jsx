...
export default class TotalBayar extends Component {
  ...
  componentDidMount() {
    ...
    // Cek apakah total_bayar sudah sesuai sebelum memanggil setState
    if (totalBayar !== this.state.total_bayar) {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.keranjang !== this.props.keranjang) {
      ...
      // Cek apakah total_bayar berubah sebelum setState
      if (totalBayar !== this.state.total_bayar) {
        this.setState({ total_bayar: totalBayar });
      }
    }
  ...
  submitTotalBayar = (event) => {
    ...
    console.log("Isi keranjang:", this.props.keranjang);
    ...
    axios
      ...
      .then((res) => {
        ...
        const detailTransaksi = this.props.keranjang.map((item) => ({
          ...
          id_produk: item.produk.id_produk, // Pastikan id_produk berasal dari produk
          kuantitas: item.kuantitas,
          total_harga: item.item.total_harga,
          nmproduk: item.item.produk.nmproduk, // Nama produk berasal dari item.produk
          harga_produk: item.produk.harga_jual, // Harga produk dari item.produk
            ...
            <Form onSubmit={this.submitTotalBayar}>
              <Col md={{ span: 3, offset: 9 }} className="px-4">
                ...
                <Form.Group controlId="id_user">
                  ...
                  <Form.Control type="text" name="id_user" value={this.state.id_user} onChange={this.handleInputChange} placeholder="Masukkan id_user" />
                </Form.Group>

                <Form.Group controlId="nama_lengkap">
                  <Form.Label>Nama Kasir</Form.Label>
                  <Form.Control type="text" name="nama_lengkap" value={this.state.nama_lengkap} onChange={this.handleInputChange} placeholder="Masukkan Nama Kasir" />
                </Form.Group>

                <Form.Group controlId="potongan">
                  <Form.Label>Potongan</Form.Label>
                  <Form.Control type="number" name="potongan" value={this.state.potongan} onChange={this.handleInputChange} />
                </Form.Group>

                <Form.Group controlId="bayar">
                  <Form.Label>Bayar</Form.Label>
                  <Form.Control type="number" name="bayar" value={this.state.bayar} onChange={this.handleInputChange} />
                </Form.Group>

                <Form.Group controlId="tanggal">
                  <Form.Label>Tanggal</Form.Label>
                  <Form.Control type="date" name="tanggal" value={this.state.tanggal} onChange={this.handleInputChange} />
                </Form.Group>

                <Form.Group controlId="metode_bayar">
                  <Form.Label>Metode Pembayaran</Form.Label>
                  <Form.Control as="select" name="metode_bayar" value={this.state.metode_bayar} onChange={this.handleInputChange}>
